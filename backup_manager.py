...
if len(backups) >= MAX_BACKUPS:
    ...
    print(f"Reached MAX_BACKUPS ({MAX_BACKUPS}), deleting {res[0][0]}")
...
with destination.backup("main", connection, "main") as backup:
    while not backup.done:
        backup.step(7)
        print(f"Copied backup pages: {backup.page_count-backup.remaining}/{backup.page_count}")

print(f'Backup created successfully as {outname}, exiting.')
